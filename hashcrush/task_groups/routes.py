...
from hashcrush.models import Rules, TaskGroups, Tasks, Users, Wordlists
...
def _owned_tasks_query():
    return Tasks.query.filter(Tasks.owner_id == current_user.id)


def _owned_task_groups_query():
    return TaskGroups.query.filter(TaskGroups.owner_id == current_user.id)


def _can_manage_task_group(task_group: TaskGroups | None) -> bool:
    return bool(task_group and (current_user.admin or task_group.owner_id == current_user.id))
...
def task_groups_list():
    ...
    task_group_rows = _visible_task_groups_query().all()
    task_rows = _visible_tasks_query().all()
    user_rows = _visible_users()
...
def task_groups_export():
    """Export owned tasks/task-groups as JSON."""
    owned_tasks = _owned_tasks_query().order_by(Tasks.id.asc()).all()
    owned_task_groups = _owned_task_groups_query().order_by(TaskGroups.id.asc()).all()

    visible_wordlists = {row.id: row.name for row in _visible_wordlists_query().all()}
    visible_rules = {row.id: row.name for row in _visible_rules_query().all()}
    owned_task_name_by_id = {task.id: task.name for task in owned_tasks}
    ...
    export_task_groups = []
    for task_group in owned_task_groups:
        task_names: list[str] = []
        for task_id in _parse_task_group_tasks(task_group.tasks):
            task_name = owned_task_name_by_id.get(task_id)
            if task_name:
...
def task_groups_import():
    """Import tasks/task-groups from JSON into current user's owned data."""
    ...
    wordlists_by_name = {row.name: row.id for row in _visible_wordlists_query().all()}
    rules_by_name = {row.name: row.id for row in _visible_rules_query().all()}

    owned_tasks = {row.name: row for row in _owned_tasks_query().all()}
    owned_task_groups = {row.name: row for row in _owned_task_groups_query().all()}
    ...
    try:
        for task_entry in tasks_payload:
            ...
            target_task = owned_tasks.get(task_name)
            if target_task:
                updated_tasks += 1
            else:
                target_task = Tasks(
            ...
            db.session.flush()
            owned_tasks[target_task.name] = target_task
        ...
        for group_entry in groups_payload:
            ...
            for task_name_entry in raw_task_names:
                ...
                if not task_name:
                    continue
                matched_task = owned_tasks.get(task_name)
                if matched_task and matched_task.id not in task_ids:
                    task_ids.append(matched_task.id)

            target_group = owned_task_groups.get(group_name)
            if target_group:
                updated_groups += 1
            else:
            ...
            target_group.owner_id = current_user.id
            target_group.tasks = str(task_ids)
            db.session.flush()
            owned_task_groups[target_group.name] = target_group
...
def task_groups_add():
    ...
    return render_template('task_groups_add.html', title='Task Group Add', tasks=task_rows, task_group_form=task_group_form)
...
def task_groups_assigned_tasks(task_group_id):
    ...
    if not _can_manage_task_group(task_group):
