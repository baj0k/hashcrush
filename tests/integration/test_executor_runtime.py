...
def test_executor_import_decodes_hashcat_autohex_plaintext(tmp_path):
    ...
    with app.app_context():
        ...
        domain = Domains(name="AutoHexDomain")
        db.session.add(domain)
        db.session.commit()

        hashfile = Hashfiles(name="autohex.txt", domain_id=domain.id)
        db.session.add(hashfile)
        db.session.commit()

        hash_row = Hashes(
            sub_ciphertext=get_md5_hash("hash-with-autohex"),
            ciphertext="hash-with-autohex",
            hash_type=1000,
            cracked=False,
            plaintext=None,
        )
        ...
        crack_path.write_text(
            "hash-with-autohex:$HEX[506173733a313233]\n",
            encoding="latin-1",
        )
        ...
        imported_hash = db.session.get(Hashes, hash_row.id)
        assert imported_hash.cracked is True
        assert decode_plaintext_from_storage(imported_hash.plaintext) == "Pass:123"
...
def test_executor_import_decodes_plain_hex_plaintext(tmp_path):
    ...
    with app.app_context():
        ...
        domain = Domains(name="PlainHexDomain")
        db.session.add(domain)
        db.session.commit()

        hashfile = Hashfiles(name="plainhex.txt", domain_id=domain.id)
        db.session.add(hashfile)
        db.session.commit()

        hash_row = Hashes(
            sub_ciphertext=get_md5_hash("hash-with-plainhex"),
            ciphertext="hash-with-plainhex",
            hash_type=1000,
            cracked=False,
            plaintext=None,
        )
        ...
        crack_path = tmp_path / "cracked.txt"
        crack_path.write_text(
            "hash-with-plainhex:506173733a313233\n",
            encoding="latin-1",
        )
